...
import { testReplicateConnection, analyzeNutritionImage, estimateNutritionWithReplicate } from '../api/replicate';

const ReplicateTestComponent: React.FC = () => {
  const [isLoading, setIsLoading] = useState(false);
  const [testResults, setTestResults] = useState<string>('');

  const runConnectionTest = async () => {
    setIsLoading(true);
    setTestResults('🧪 Test de connexion Replicate...\n');
    
    try {
      const success = await testReplicateConnection();
      setTestResults(prev => prev + `✅ Connexion: ${success ? 'Réussie' : 'Échouée'}\n`);
    } catch (error) {
  ...
  const runNutritionEstimateTest = async () => {
    ...
    try {
      const result = await estimateNutritionWithReplicate('chicken breast');
  ...
  const runFullTest = async () => {
    setTestResults('🚀 Début des tests complets Replicate...\n');
  ...
  return (
    <ScrollView className="flex-1 bg-white p-4">
      <Text className="text-xl font-bold text-gray-800 mb-4">Test Replicate GPT-4.1-mini</Text>
...
export default ReplicateTestComponent;
