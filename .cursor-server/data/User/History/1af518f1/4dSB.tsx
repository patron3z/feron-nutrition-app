  ...
  return (
    <SafeAreaView className="flex-1 bg-white">
      ...
      <View className="mx-6 mt-4">
        <Text className="text-gray-800 font-semibold mb-2">Meal type</Text>
              ...
              <View className="flex-row items-start justify-between">
                <View className="flex-1 pr-3">
                  ...
                  <View className="flex-row mt-2">
                    {[0.5, 1, 1.5].map(val => (
                      <Pressable key={val} className={`px-3 py-1 rounded-full mr-2 ${((scales[f.id]||1) === val) ? "bg-purple-600" : "bg-gray-100"}`} onPress={() => setScales(prev => ({ ...prev, [f.id]: val }))}>
                        <Text className={`${((scales[f.id]||1) === val) ? "text-white" : "text-gray-700"}`}>x{val}</Text>
                      </Pressable>
                    ))}
                  </View>
                </View>
      ...
      <View className="px-6 pb-6">
        <Pressable className="bg-green-500 rounded-xl py-4 items-center" onPress={onAdd}>
          <Text className="text-white font-semibold text-lg">Add to log</Text>
        </Pressable>
      </View>
    </SafeAreaView>
  );
