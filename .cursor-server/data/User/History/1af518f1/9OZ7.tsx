...
export default function ConfirmFoodsScreen({ navigation }: any) {
  ...
  const isEditing: boolean = route.params?.isEditing || false;
  const originalMealName: string | undefined = route.params?.originalMealName;

  // Debug log
  console.log('🍽️ ConfirmFoods - Initial foods:', initialFoods.length);
  console.log('🍽️ ConfirmFoods - Route params:', route.params);
  ...
  const onAdd = () => {
    ...
    const meal: Meal = {
      ...
      name: isEditing && originalMealName 
        ? originalMealName 
        : `Scan - ${selectedFoods.map(f => f.name).join(', ')}`,
      foods: mealFoods,
    ...
    <SafeAreaView className="flex-1 bg-white">
      ...
      <View className="px-6 py-4 border-b border-gray-100 flex-row items-center justify-between">
        ...
        <Text className="text-lg font-semibold text-gray-800">
          {isEditing ? "Edit meal" : "Confirm foods"}
        </Text>
        <View className="w-6" />
      </View>
        ...
        <View className="mx-6 mt-4">
          {initialFoods.length === 0 ? (
            <View className="items-center py-16">
              ...
              <Text className="text-gray-500 mt-2 text-center">Aucun aliment détecté</Text>
              <Text className="text-gray-400 text-sm mt-1 text-center">Vérifiez que l'analyse s'est bien déroulée</Text>
            </View>
          ) : (
            initialFoods.map((f) => (
      ...
      <View className="absolute bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-6 py-4">
        <Pressable 
          className="bg-green-500 rounded-xl py-4 items-center flex-row justify-center shadow-lg" 
          onPress={onAdd}
          disabled={initialFoods.filter(f => (quantities[f.id] || 0) > 0).length === 0}
        >
          <Ionicons name={isEditing ? "save" : "checkmark-circle"} size={24} color="white" />
          <Text className="text-white font-semibold text-lg ml-2">
            {isEditing ? "Save changes" : `Confirm (${initialFoods.filter(f => (quantities[f.id] || 0) > 0).length} items)`}
          </Text>
        </Pressable>
      </View>
    </SafeAreaView>
