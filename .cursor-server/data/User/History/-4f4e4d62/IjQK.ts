    ...
    for (const foodWithScale of parsedFoods) {
      ...
      try {
        ...
        if (Array.isArray(usdaResults) && usdaResults.length > 0) {
          ...
          console.log(`⚠️ Pas de données USDA pour ${foodName}, utilisation Replicate avec échelle ${scale}x`);
          
          try {
            const nutritionData = await estimateNutritionWithReplicate(foodName);
            
            // Utiliser les valeurs Replicate ou des valeurs par défaut améliorées
