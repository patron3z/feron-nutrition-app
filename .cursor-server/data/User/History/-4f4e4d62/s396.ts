...
/**
 * Enhanced food analysis using USDA + OpenAI GPT-4o with portion scale detection
 */
export const analyzeFoodWithUSDAAndReplicate = async (imageBase64?: string, description?: string): Promise<NutritionAnalysisResult> => {
    console.log('🔍 Analyse avancée avec USDA + OpenAI GPT-4o + Scale...');
  
  try {
    // Step 1: Use OpenAI GPT-4o to identify foods AND estimate portion scales
...
Rice | 1.0 | Typical portion size`;
    ...
    const cleanResponse = (replicateResponse || "")
      ...
      .replace(/\r/g, "")
      .trim();
    console.log('🤖 OpenAI GPT-4o identification avec scale:', cleanResponse);
    ...
    if (cleanResponse.toLowerCase().includes('no food') || 
      ...
      console.warn('⚠️ OpenAI ne détecte pas de nourriture dans l\'image');
      console.log('💡 Suggestions: Essayez avec une image plus claire ou mieux éclairée');
    }

    // Step 2: Parse OpenAI response to extract foods and scales
        ...
        if (Array.isArray(usdaResults) && usdaResults.length > 0) {
          ...
          try {
            ...
            nutritionResults.push({
              id: `openai_${Date.now()}_${foodName.replace(/\s+/g, '_')}_${scale}x`,
              name: foodName,
            ...
            console.log(`✅ OpenAI estimation avec échelle ${scale}x pour ${foodName}: ${(baseCalories * scale).toFixed(0)}kcal`);
          } catch (parseError) {
            console.warn('Erreur parsing OpenAI nutrition, utilisation des valeurs par défaut améliorées:', parseError);
    ...
    return {
      ...
      suggestions: [
        "Données provenant de USDA et OpenAI GPT-4o avec détection d'échelle",
    ...
    console.error('❌ Erreur analyse USDA + OpenAI:', error);
