    ...
    for (const foodWithScale of parsedFoods) {
      ...
      try {
        ...
        if (Array.isArray(usdaResults) && usdaResults.length > 0) {
          ...
          const estimatePrompt = `Provide realistic nutritional information for "${foodName}" per 100g in JSON format: {"calories": number, "protein": number, "carbs": number, "fat": number, "fiber": number, "sugar": number, "sodium": number}. Be accurate based on the food type.`;
          ...
          try {
            ...
            // Utiliser les valeurs Gemini ou des valeurs par défaut améliorées
            ...
            console.log(`✅ Gemini estimation avec échelle ${scale}x pour ${foodName}: ${(baseCalories * scale).toFixed(0)}kcal`);
          } catch (parseError) {
            console.warn('Erreur parsing Gemini nutrition, utilisation des valeurs par défaut améliorées:', parseError);
