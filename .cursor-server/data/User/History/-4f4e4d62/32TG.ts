...
export const analyzeFoodWithUSDAAndGemini = async (imageBase64?: string, description?: string): Promise<NutritionAnalysisResult> => {
  ...
  try {
    ...
    if (imageBase64) {
      identificationPrompt = `Analyze this food image and provide detailed information about each visible food item. For each food, estimate:
1. Food name and cooking method
2. Portion size scale compared to standard serving (0.5 = half portion, 1.0 = standard, 2.0 = double, etc.)
3. Visual cues used for scale estimation (plate size, utensils, hand reference, etc.)

Respond in this format for each food:
FOOD_NAME | SCALE_FACTOR | SCALE_REASONING

Example:
Grilled chicken breast | 1.2 | Appears larger than standard 100g serving based on plate proportion
Rice | 0.8 | About 3/4 cup visible, smaller than standard 1 cup serving
Broccoli | 1.5 | Large portion covering significant plate area
