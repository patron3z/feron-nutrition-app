...
import { analyzeNutritionImage, estimateNutritionWithReplicate } from '../api/replicate';
...
const OpenAITestComponent: React.FC = () => {
  ...
  const runConnectionTest = async () => {
    ...
    try {
      ...
      await estimateNutritionWithReplicate('apple');
  ...
  const runImageAnalysisTest = async () => {
    ...
    try {
      // Test avec une image d'exemple (base64 ou URL)
      ...
      const result = await analyzeNutritionImage(testImageData);
      setTestResults(prev => prev + `✅ Analyse image: ${result.substring(0, 200)}...\n`);
    } catch (error) {
